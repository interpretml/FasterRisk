...
class RiskScoreOptimizer:
    def __init__(
        ...
        assert (
            max(np.abs(y_unique - y_unique_expected)) < 1e-8
        ), "input y must be equal to only +1 or -1"
        assert len(X_shape) == 2, "input X must have 2-D shape!"
        assert (
            X_shape[0] == y_shape[0]
        ), "number of rows from input X must be equal to the number of elements from input y!"
        self.y = y
        self.X = X
        ...
        if self.group_sparsity is None:
            self.sparseLogRegModel_object = sparseLogRegModel(
                X, y, intercept=True, original_lb=lb, original_ub=ub
            )
            self.sparseDiversePoolLogRegModel_object = sparseDiversePoolLogRegModel(
                X, y, intercept=True, original_lb=lb, original_ub=ub
            )
        else:
            ...
            assert (
                self.featureIndex_to_groupIndex is not None
            ), "featureIndex_to_groupIndex must be provided if group_sparsity is not None"
            assert (
                type(self.featureIndex_to_groupIndex[0]) == np.int_
            ), "featureIndex_to_groupIndex needs to be a NumPy integer array"

            self.groupIndex_to_featureIndices = get_groupIndex_to_featureIndices(
                self.featureIndex_to_groupIndex
            )

            self.sparseLogRegModel_object = groupSparseLogRegModel(
                X,
                y,
                intercept=True,
                original_lb=lb,
                ...
                groupIndex_to_featureIndices=self.groupIndex_to_featureIndices,
            )
            self.sparseDiversePoolLogRegModel_object = (
                groupSparseDiversePoolLogRegModel(
                    X,
                    y,
                    intercept=True,
        ...
        self.starRaySearchModel_object = starRaySearchModel(
            X=X,
            y=y,
            lb=lb,
            ub=ub,
            num_ray_search=num_ray_search,
            early_stop_tolerance=lineSearch_early_stop_tolerance,
        )
    ...
    def optimize(self):
        ...
        self.sparseLogRegModel_object.get_sparse_sol_via_OMP(
            k=self.k, parent_size=self.parent_size, child_size=self.child_size
        )

        beta0, betas, ExpyXB = self.sparseLogRegModel_object.get_beta0_betas_ExpyXB()
        self.sparseDiversePoolLogRegModel_object.warm_start_from_beta0_betas_ExpyXB(
            beta0=beta0, betas=betas, ExpyXB=ExpyXB
        )
        sparseDiversePool_beta0, sparseDiversePool_betas = (
            self.sparseDiversePoolLogRegModel_object.get_sparseDiversePool(
        ...
        (
            self.multipliers,
            self.sparseDiversePool_beta0_integer,
            self.sparseDiversePool_betas_integer,
        ) = self.starRaySearchModel_object.star_ray_search_scale_and_round(
            sparseDiversePool_beta0, sparseDiversePool_betas
        )

    def _sort_IntegerPool_on_logisticLoss(self):
        """sort the integer solutions in the pool by ascending order of logistic loss"""
        sparseDiversePool_XB = (
            self.sparseDiversePool_beta0_integer.reshape(1, -1)
            + self.X @ self.sparseDiversePool_betas_integer.transpose()
        ) / (self.multipliers.reshape(1, -1))
        ...
        sparseDiversePool_logisticLoss = np.sum(
            np.log(1.0 + np.reciprocal(sparseDiversePool_ExpyXB)), axis=0
        )
        orderedIndices = np.argsort(sparseDiversePool_logisticLoss)

        self.multipliers = self.multipliers[orderedIndices]
        self.sparseDiversePool_beta0_integer = self.sparseDiversePool_beta0_integer[
            orderedIndices
        ]
        self.sparseDiversePool_betas_integer = self.sparseDiversePool_betas_integer[
            orderedIndices
        ]
    ...
    def get_models(self, model_index=None):
        ...
        if self.multipliers is None:
            raise ValueError(
                "Please run the optimization first by calling the function .optimize()"
            )
        ...
        if model_index is not None:
            return (
                self.multipliers[model_index],
                self.sparseDiversePool_beta0_integer[model_index],
                self.sparseDiversePool_betas_integer[model_index],
            )
        return (
            self.multipliers,
            self.sparseDiversePool_beta0_integer,
    ...
    def get_models_in_dict(
        self, featureNames, model_index=None, featureIndex_to_groupIndex=None
    ):
        ...
        if self.featureIndex_to_groupIndex is None:
            if featureIndex_to_groupIndex is None:
                raise ValueError(
                    "featureIndex_to_groupIndex must be provided if it is not provided during initialization"
                )
            self.featureIndex_to_groupIndex = featureIndex_to_groupIndex
        else:
            if featureIndex_to_groupIndex is not None:
                assert isEqual_upTo_8decimal(
                    self.featureIndex_to_groupIndex, featureIndex_to_groupIndex
                ), "featureIndex_to_groupIndex provided during initialization is different from the one provided now"
        ...
        if model_index is not None:
            ...
            for feature_index in nonzero_indices:
                feature_data_list.append(
                    [coefficients[feature_index], featureNames[feature_index]]
                )

            # obtain risk scale
            all_scores, all_risks = RiskScoreClassifier_m._get_scores_and_risks(
                quantile_len=20
            )
            risk_scale_list = []
            ...
            train_logistic_loss = RiskScoreClassifier_m.compute_logisticLoss(
                self.X, self.y
            )
            ...
            model_in_dict["card_label"] = str(model_index + 1).zfill(2)
        ...
        for model_index in range(len(multipliers)):
            model_in_dict_list.append(
                self.get_models_in_dict(
                    featureNames, model_index, featureIndex_to_groupIndex
                )
            )
...
class RiskScoreClassifier:
    def __init__(
        self, multiplier, intercept, coefficients, featureNames=None, X_train=None
    ):
    ...
    def predict(self, X):
        ...
        """
        y_score = (
            self.intercept + X.dot(self.coefficients)
        ) / self.multiplier  # numpy dot.() has some floating point error issues, so we avoid using self.scaled_intercept and self.scaled_coefficients directly
    ...
    def compute_logisticLoss(self, X, y):
        ...
        """
        return compute_logisticLoss_from_X_y_beta0_betas(
            X, y, self.scaled_intercept, self.scaled_coefficients
        )
    ...
    def get_acc_and_auc(self, X, y):
        ...
        fpr, tpr, thresholds = sklearn.metrics.roc_curve(
            y_true=y, y_score=y_pred_prob, drop_intermediate=False
        )
    ...
    def _print_score_calculation_table(self):
        assert (
            self.featureNames is not None
        ), "please pass the featureNames to the model by using the function .reset_featureNames(featureNames)"

        nonzero_indices = get_support_indices(self.coefficients)

        max_feature_length = max(
            [len(featureName) for featureName in self.featureNames]
        )
        row_score_template = "{0}. {1:>%d}     {2:>2} point(s) | + ..." % (
            max_feature_length
        )

        print("The Risk Score is:")
        for count, feature_i in enumerate(nonzero_indices):
            row_score_str = row_score_template.format(
                count + 1,
                self.featureNames[feature_i],
                int(self.coefficients[feature_i]),
            )
            if count == 0:
    ...
    def _print_score_risk_table(self, quantile_len):
        ...
        self._print_score_risk_row(
            all_scores[:num_scores_div_2], all_risks[:num_scores_div_2]
        )
        self._print_score_risk_row(
            all_scores[num_scores_div_2:], all_risks[num_scores_div_2:]
        )
